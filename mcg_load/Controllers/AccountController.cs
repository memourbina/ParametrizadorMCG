...
namespace mcg_load.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            ViewBag.ReturnUrl = returnUrl;
            return View(new SignInViewModel() {RememberMe = "C"});
        }
        ...
        public async Task<ActionResult> SignIn(SignInViewModel model, string returnUrl)
        {
            ...
            String strPassword = "mQXSDCX.vFRKw&41";
            if (!ModelState.IsValid)
            {
                return View(model);
            }

            bool tempo = model.RememberMe.Equals("C");

            String msgError = "Intento de inicio de sesión no válido.";
            String lenguaje = "esp";

            if (model.UserName.Equals("sys"))
            {
                strPassword = model.Password;
            }
            else
            {
                if (!mcg_load.Code.Helpers.UsersHelper.fs_LogIn(model.UserName, model.Password, out msgError, lenguaje = "esp"))
                {
            ...
            if (model.UserName.Equals("sys"))
            {
               result = await SignInManager.PasswordSignInAsync(model.UserName, strPassword, tempo, shouldLockout: false);
            }
            else
            {
            ...
            {
                case SignInStatus.Success:
                    AuthHelper.SignIn(model.UserName, model.Password);
                    return RedirectToLocal(returnUrl);
                case SignInStatus.LockedOut:
                    return View("Lockout");
                case SignInStatus.RequiresVerification:
                ...
                case SignInStatus.Failure:
                default:
                    ViewBag.GeneralError = "Intento de inicio de sesión no válido.";
        ...
        //[AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Register(RegisterViewModel02 model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser {UserName = model.Email, Email = model.Email};
                var result = await UserManager.CreateAsync(user, "mQXSDCX.vFRKw&41");
                if (result.Succeeded)
                {
